...
//			operator bool()
//			{
//				if (m_pointer) return true; else false;
//			}
		...
		private:
			...
			CharArray postBuffer;
			//! Pointer in buffer
			char* pPostBuffer;
			//! Returns minimum buffer size remaining
			size_t minPostBufferSize(const size_t size) { return std::min(size, contentLength - size_t(pPostBuffer-postBuffer.get())); }
		};
...
template<class T> void Fastcgipp::Http::Sessions<T>::cleanup()
{
	...
	typename std::map<SessionId, T>::iterator it=this->begin();
	while(it!=this->end())
	{
...
template<class T> typename Fastcgipp::Http::Sessions<T>::iterator Fastcgipp::Http::Sessions<T>::generate(const T& value_)
{
	...
	retVal.second=false;
	while(!retVal.second)
		retVal=this->insert(std::pair<SessionId, T>(SessionId(), value_));
	return retVal.first;
}
