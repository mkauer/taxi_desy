	...
	namespace Protocol
	{
		...
		public:
			...
			void setRequestId(RequestId requestId_) {
				requestId=readBigEndian(requestId_);
			}
			...
			RequestId getRequestId() const {
				return readBigEndian(requestId);
			}
			...
			void setContentLength(uint16_t contentLength_) {
				contentLength=readBigEndian(contentLength_);
			}
			...
			int getContentLength() const {
				return readBigEndian(contentLength);
			}
		...
		public:
			 ...
			 */
			Role getRole() const { return static_cast<Role>(readBigEndian(role)); }
