...
template<class charT> bool Fastcgipp::Http::Environment<charT>::fillPostBuffer(const char* data, size_t size)
{
	if(!postBuffer.get())
	{
		DBG("allocate postBuffer contentLength: " << contentLength);
		postBuffer.reset(new char[contentLength]);
		pPostBuffer=postBuffer.get();
	} else {
		DBG("postBuffer already allocated! " << postBuffer << "," << ((int)postBuffer.get()) <<  " " << contentLength);
	}

	size_t trueSize=minPostBufferSize(size);
	if(trueSize)
	{
		DBG("memcpy " << pPostBuffer << "," << data << "," << trueSize);
...
template<class charT> void Fastcgipp::Http::Environment<charT>::parsePostsUrlEncoded()
{
	...
	// FIX if no post data exists, just exit
	// this can happen with empty POST forms...
//	if (!postBuffer.get()) return;
