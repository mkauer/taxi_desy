...
{
	size_t events = _words / TAXI_FIFO_WIDTH_WORDS;
	...
	for (int ev = 0; ev < events; ev++)
	{
		...
		for (int i = 0; i < TAXI_FIFO_WIDTH_WORDS; i++)
		{
			int data = _data[i + ev*TAXI_FIFO_WIDTH_WORDS];
...
int getEvent(uint16_t* _data, uint32_t* _eventCounter, size_t* _words)
{
	...
	*_words = 0;
	...
	if(( *(_data+0) == 0x1000) || (*(_data+0) == 0x2000) || (*(_data+0) == 0x3000) || (*(_data+0) == 0x4000) || (*(_data+0) == 0x5000) || (*(_data+0) == 0x6000) || (*(_data+0) == 0x7000))
	{
		...
		length = 1;// *(_data+3);
		*_words += TAXI_FIFO_WIDTH_WORDS;

//		for(int i=1;i<length;i++)
//		{
//			if(*(_data+i*TAXI_FIFO_WIDTH_WORDS) != (0xc000+i-1))
//			{
////				error++;
//			}
//
//			for (int j=0;j<8;j++)
//			{
//				if(*(_data+i*TAXI_FIFO_WIDTH_WORDS+j+1) != ((i-1)*8+j))
//				{
////					error++;
//				}
//			}
